# Configuration Guide <br/> PayrollChecks service Microservice
...
Example:
```yaml
- descriptor: "service-payrollchecks:persistence:memory:default:1.0"
  options:
    max_page_size: 100
```
...
- path: string - file path where PayrollCheckV1 objects are stored. The object are written into the file in JSON format.
- options: object - Misc configuration options
  - max_page_size: number - Maximum number of items per page (default: 100)
